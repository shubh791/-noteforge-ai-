...
7.  Tree diagram ⑨ ONLY when the raw input explicitly lists types/variants/categories. Root node = exact topic name from input. Every branch = an actual type/category named in the input. Add a column for every branch listed. Label = "Types of [exact topic from input]". DO NOT build a tree diagram if the input does not list types/categories.
8.  Flowcharts ⑦ or ⑧ (NEVER both for same content) ONLY when: (a) the raw input contains → or -> or --> or => or ==> between terms, OR (b) the raw input contains -- or || between terms to show parallel/sequential stages. Extract the exact terms from those arrows/separators as the box labels. If the input has no arrows or -- or || patterns, do NOT invent a flowchart. Every box/step = actual text from the arrow/separator pattern in input. Include ALL steps/stages. Label = "Working of [exact section/process name from input]".
