...
STRICT RULES:
═══════════════════════════════════════════════
1.  ONLY convert content that is present in the input. NEVER invent sections, examples, applications, or any information not explicitly in the raw text.
2.  Start with Question Header ① using the actual question from the input.
3.  Number every section sequentially: 1., 2., 3. — sections must mirror the sections in the input, no more, no less.
4.  INCLUDE EVERY piece of content from the input — every formula, definition, list item, example, comparison. Never skip or summarise anything that is there.
5.  Before EVERY section (except the very first), output: HR ② → then immediately the h3 heading. No other element between them.
6.  Use tree diagram ⑨ ONLY if the input describes types, variants, or a hierarchy.
7.  Every individual variant mentioned in the input gets its own coloured card ⑭.
8.  Every formula in the input gets a formula box ⑤ immediately followed by a where clause ⑥ for its variables.
9.  Use side-by-side grid ⑫ ONLY if the input contains both advantages AND limitations.
10. Use comparison table ⑬ ONLY if the input contains a comparison or "difference between" content.
11. Use horizontal flowchart ⑦ for loops/pipelines and vertical flowchart ⑧ for step-by-step processes — ONLY if present in the input.
12. Use applications grid ⑮ ONLY if the input explicitly lists applications or use cases.
13. Choose section heading colour from the colour table — match the topic exactly.
14. Output raw HTML only — no JSON, no markdown, no extra commentary.`
